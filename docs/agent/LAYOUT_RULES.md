...
| **DOT-003** | Fusion splice dot sits on a **horizontal** segment (**DOT-001**) and is **≥48px** along the leg path from the nearest leg **corner** (90° bend). Measured by walking backward on the left leg and forward on the right leg from the dot. Manual segment moves must respect the same clearance (`constraints.ts`). |
